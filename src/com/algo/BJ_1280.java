 ...
 *
 * i 의 비용
 *  (  i 의 왼족에 있는 나무의 수 * i 까지의 거리 ) - (i 앞에 있는 나무들의 0 에서 나무까지의 거리 누적합)
 *                              +
...
public class BJ_1280 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < N; i++) {
            ...
            if (i != 0) mul  = mul.multiply(BigInteger.valueOf(result)).mod(BigInteger.valueOf(mod));
        }
    ...
    public static long getCNT(int L, int R, int idx, int temp_L, int temp_R) {
        ...
        if (L <= temp_L && temp_R <= R) return cnt_tree[idx];
    ...
    public static long getSum(int L, int R, int idx, int temp_L, int temp_R) {
        if (temp_L > R || temp_R < L) return 0;
        if (L <= temp_L && temp_R <= R) return sum_tree[idx];
        int mid = (temp_L + temp_R) / 2;
